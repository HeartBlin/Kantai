...
let
  ...
  roleImport = if role == "desktop" then
    [ ./roles/desktop ]
  else if role == "laptop" then
    [ ./roles/laptop ]
  ...
  coreImports =
    [ ./boot ./git ./hardware/nvidia.nix ./id ./nix ./pfp ./security ];
in {
  config.assertions = [
    # Role enforcement
    {
    ...
    {
      assertion = ((role != "laptop" || !nvidia.enable) || (nvidia.busIDs.amd
        != "PCI:0:0:0" && nvidia.busIDs.nvidia != "PCI:0:0:0"));
      message = ''
        Laptop NVIDIA configuration error:
          - Must set NVIDIA GPU ID
          - Must set one integrated GPU (AMD for now)
          Current values:
          - NVIDIA: ${nvidia.busIDs.nvidia} ${
            lib.optionalString (nvidia.busIDs.nvidia == "PCI:0:0:0") "(unset)"
          }
          - AMD: ${nvidia.busIDs.amd} ${
            lib.optionalString (nvidia.busIDs.amd == "PCI:0:0:0") "(unset)"
          }
      '';
    }
  ];
